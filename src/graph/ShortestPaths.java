    ...
    public void singleSourceDistances(Vertex source) {
        ...
        try{
            ...
            //    System.out.println(g);
                for(Edge edge: graph.outgoingEdges(g)){
                    Vertex v = graph.dest(edge);
                   // System.out.println(v);
                    if(!distances.containsKey(v)){
                        // && getDistance(v) == Double.POSITIVE_INFINITY)
                       // Edge eg = (Edge) edge;
                      //  distances.put(v, getDistance(g) + graph.weight(edge));
                   //     if(!distances.containsKey(g)) distances.put(v, graph.weight(edge));
                    //    else{
                        distances.put(v, distances.get(g) + graph.weight(edge));
                    //    }
                        frontier.add(v, distances.get(v));
                        bestEdges.put(v, edge);
                    }
                    else{
                        if (distances.get(g) + graph.weight(edge) < distances.get(v)){
                            distances.replace(v, distances.get(g) + graph.weight(edge));
                            frontier.changePriority(v, distances.get(v));
                            bestEdges.replace(v, edge);
                        }
                    }
                }
            }
